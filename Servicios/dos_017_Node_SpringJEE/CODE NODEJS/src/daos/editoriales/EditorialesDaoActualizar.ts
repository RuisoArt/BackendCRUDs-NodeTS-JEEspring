...
class EditorialesDaoActualizar{
    public static async actualizarEditorial(sqlActualizar:string, parametros:any, res:Response):Promise<any>{
        await pool.task(async (consulta: any)=>{
            return await consulta.result(sqlActualizar, parametros);
        })
        .then((respuesta: any)=>{
            console.log(respuesta);
            res.status(200).json({respuesta: 'Editorial Actualizada'});
        })
        .catch((miError:any)=>{
...
export default EditorialesDaoActualizar;